...
export default async function fornecedoresRoutes(fastify) {
  ...
  fastify.get("/", async () => {
    return await fornecedoresController.listar()
  })
  fastify.get("/listar", async () => {
    return await fornecedoresController.listar()
  })
  fastify.get("/:id", async (request) => {
    return await fornecedoresController.buscarPorId(request.params.id)
  })
  fastify.post("/", async (request, reply) => {
    const categoria = await fornecedoresController.criar(request.body)
    return reply.code(201).send(categoria)
  })
  fastify.put("/:id", async (request, reply) => {
    const categoria = await fornecedoresController.atualizar(request.params.id, request.body)
    return reply.code(200).send(categoria)
  })
  fastify.delete("/:id", async (request, reply) => {
    await fornecedoresController.excluir(request.params.id)
